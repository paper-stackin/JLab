...
void MomentumBinsGraphs(
    T* (&hists)[N], 
    TH2* h2,
    double hist_fit_X_max, 
    ...
    double fit_x_max)
{
    ...
    std::vector<double> p_vals, left_vals, right_vals;
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        double p = 0.25 * (i + 0.5);
        p_vals.push_back(p);
        left_vals.push_back(left);
        right_vals.push_back(right);
    }

    TGraph* g_left  = new TGraph(p_vals.size(), p_vals.data(), left_vals.data());
    TGraph* g_right = new TGraph(p_vals.size(), p_vals.data(), right_vals.data());

    g_left->RemovePoint(0);
    g_right->RemovePoint(0);

    g_left->SetMarkerStyle(20);
    g_right->SetMarkerStyle(20);

    g_left->SetMarkerColor(kRed);
    g_right->SetMarkerColor(kRed);

    // --- создаём полиномы ---
    TF1* f_left  = new TF1("f_left",  "pol5", 0, hist_fit_X_max);
    TF1* f_right = new TF1("f_right", "pol5", 0, hist_fit_X_max);

    // --- фит ---
    g_left->Fit(f_left, "R");
    g_right->Fit(f_right, "R");
    ...
    // --- линии фита ---
    f_left->SetLineColor(kRed);
    f_right->SetLineColor(kRed);
...
void delta_t_cut(void)
{
    ...
    MomentumBinsGraphs(p_delta_t_FD, p_delta_t_vs_p_FD, 5, "FD_proton_delta_t.pdf", -0.3, 0.2);
    MomentumBinsGraphs(p_delta_t_CD, p_delta_t_vs_p_CD, 2.7, "CD_proton_delta_t.pdf", -0.3, 0.2);
    MomentumBinsGraphs(pim_delta_t_FD, pim_delta_t_vs_p_FD, 5, "FD_pim_delta_t.pdf", -0.3, 0.3);
    MomentumBinsGraphs(pim_delta_t_CD, pim_delta_t_vs_p_CD, 2.7, "CD_pim_delta_t.pdf", -0.25, 0.25);
