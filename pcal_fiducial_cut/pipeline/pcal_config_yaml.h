...
namespace YAML {
    ...
    struct convert<FileConfig>
    {
        ...
        {
            cfg.input_file = node["input_file"].as<std::string>();
            cfg.intermediate_file = node["intermediate_file"].as<std::string>();
            cfg.pdf_name_prefix = node["pdf_name_prefix"].as<std::string>();
    ...
    struct convert<HistogramConfig>
    {
        static bool decode(const Node& node, HistogramConfig& cfg)
        {
            ...
            cfg.title_prefix    = node["title_prefix"].as<std::string>();
            cfg.title_suffix    = node["title_suffix"].as<std::string>();
            cfg.title_2d_prefix = node["title_2d_prefix"].as<std::string>();
            cfg.title_2d_suffix = node["title_2d_suffix"].as<std::string>();
    ...
    struct convert<BoundaryConfig>
    {
        static bool decode(const Node& node, BoundaryConfig& cfg)
        {
            ...
            cfg.fit_formula     = node["fit_formula"].as<std::string>();
