...
TGraph* DrawGraphWithLines(const std::vector<double>& x_vals, const std::vector<double>& y_vals,
                           Color_t color)
{
    // Создаем график по точкам
    TGraph* graph = new TGraph(x_vals.size(), x_vals.data(), y_vals.data());
    
    // Настраиваем маркеры (сами точки)
    graph->SetMarkerStyle(20);
    graph->SetMarkerColor(color);
    
    // Настраиваем линию, которая соединит точки
    graph->SetLineColor(color);
    graph->SetLineWidth(2); // Опционально: делает линию чуть толще

    // Опция "PL same" рисует и точки (P), и линии между ними (L)
    graph->Draw("PL same");
...
void pcal_pdf(const char* config_file)
{
    ...
    for (int sec = 0; sec < 6; ++sec)
    {
        ...
        std::pair<TGraph*, TF1*> lower_result = DrawGraphWithFit(x_vals, lower_vals, cfg.boundary.fit_formula.c_str(), cfg.boundary.x_fit_min, cfg.boundary.x_fit_max, kRed, "f_lower");
        std::pair<TGraph*, TF1*> upper_result = DrawGraphWithFit(x_vals, upper_vals, cfg.boundary.fit_formula.c_str(), cfg.boundary.x_fit_min, cfg.boundary.x_fit_max, kRed, "f_upper");

        // TGraph* g_lower = DrawGraphWithLines(x_vals, lower_vals, kRed);
        // TGraph* g_upper = DrawGraphWithLines(x_vals, upper_vals, kRed);
