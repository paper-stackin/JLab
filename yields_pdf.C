...
void yields_pdf(void) 
{
	...
	TFile *infile_exp = TFile::Open("MMp_hists_Mp.root"); // 1 method
	TFile *infile_ann_good = TFile::Open("MMp_hists_M0.root"); // 4th method
...
/////////////////////////////////////////////////////////////////////// Background fit/////////////////////////////////////////////////////
	...
	for (int q = 0; q < q_fin; ++q) 
    {
	    ...
	    for (int w = 0; w < w_brink; ++w) 
        {
	        ...
	        MM_raw[q][w] = (TH1F*)infile_exp -> Get(Form("%s", namehist_raw));
	...
	for (int q = 0; q < q_fin; ++q) 
    {
	    ...
	    for (int w = 0; w < w_brink; ++w) 
        {
            ...
            else 
            {
                ...
                TLine *l_3 = new TLine(0.01948816, 0, 0.01948816, MM_raw[q][w] -> GetMaximum());
                ...
                pave -> AddText(Form("   Total events within e. c.: %.0f ", MM_raw[q][w] -> Integral(25, 40)));
                /*pave -> AddText(Form("   N_e.c. - N_bg: %.0f ", MM_raw[q][w] -> Integral(25, 40) - bg -> Integral(-0.05, 0.1) * 100));
                pave -> AddText(Form("Fit results:"));
...
/////////////////////////////////////////////////////////////////////// Background fit for MM0 case //////////////////////////////////////////////////////////////////////////////
	...
	for (int q = 0; q < q_fin; ++q) 
    {
	    ...
	    for (int w = 0; w < w_brink; ++w) 
        {
	        ...
	        MM_ann_good_4th_curve[q][w] = (TH1F*)infile_ann_good -> Get(Form("%s", namehist_ann));
...
// ////////////////////////////////////////////////////////////////////////////////4th method - scaling MM0 BG/////////////////////////////////////////////////////////////////////
	...
	for (int q = 0; q < q_fin; ++q) 
    {
        ...
        {      
	        ...
	        MM_my_4th_curve[q][w] = (TH1F*)infile_exp -> Get(Form("%s", namehist_raw));
            ...
            else 
            {
                ...
                TLine *l_3 = new TLine(0.01948816, 0, 0.01948816, MM_ann_good_4th_curve[q][w] -> GetMaximum());
                l_3 -> SetLineColor(kBlack);
	            l_3 -> Draw("SAME");
                l_3 -> SetLineStyle(kDashed);
                ...
                pave -> AddText(Form("   Total events within e. c.: %.0f ", MM_ann_good_4th_curve[q][w] -> Integral(25, 40)));
                pave -> AddText(Form("   N_{e.c.} - N_{bg}: %.0f ", MM_ann_good_4th_curve[q][w] -> Integral(25, 40) - bg -> Integral(-0.05, 0.1) * 100));
                pave -> AddText(Form("Fit results:"));
                pave -> AddText(Form("   Amplitude bg: %.5f +/- %.5f", bg -> GetParameter(0), bg -> GetParError(0)));
                pave -> AddText(Form("   Mean bg: %.5f +/- %.5f", bg -> GetParameter(1), bg -> GetParError(1)));
                pave -> AddText(Form("   Sigma bg: %.5f +/- %.5f", bg -> GetParameter(2), bg -> GetParError(2)));
...
/////////////////////////////////////////////////////////////////////////////////GRAPHS/////////////////////////////////////////////////////////////////////////////
    ...
    for (int q = 0; q < 6; ++q) 
    {
	    ...
	    char namegraph[256];
        sprintf(namegraph, "Yield VS W for Q^{2} in [%g, %g] GeV^{2}; W, GeV; Yield", Q2_min_val, Q2_max_val);
