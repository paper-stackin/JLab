...
void MPPT_events_simulation_selection_with_additional_values_saved()
{
    ...
    int ppindex, pimpindex, pippindex; //indexes of particles in event
    ...
    float DCexR1_new, DCeyR1_new;
    float DCexR2_new, DCeyR2_new;
    float DCexR3_new, DCeyR3_new;
    ...
    float time_e, time_p, time_pim; // Measured time in SC
    ...
    // Initializing missing pi+ histograms for different Q^2 and W bins
    TH1F *MM[12][70];
    for (int q = 0; q < 6; ++q) 
    {
        for (int w = 0; w < 64; ++w)
    ...
    tree.Branch("time_p", &time_p, "time_p");
    tree.Branch("path_p", &path_p, "path_p");
    ...
    tree.Branch("time_pim", &time_pim, "time_pim");
    tree.Branch("path_pim", &path_pim, "path_pim");
    ...
    TLorentzVector el, pim, pfin;
    TLorentzVector prec, piprec, pimrec, Sum; // Might delete later

    int counter = 0;

    // Forming data file name
    std::vector<string> data;
    string str = "/volatile/clas12/osg/savkin/10717/10717-";
    string fin = ".hipo";
    for (int i = 0; i <= 9999; i++)
    {
        if (i == 2622) continue; // Can't open this file
        if (i == 3076) continue; // Can't open this file
        if (i == 3943) continue; // Can't open this file
        if (i == 4725) continue; // Can't open this file
        data.push_back(str + std::__cxx11::to_string(i) + fin);
    } 
    ...
    for(int r = 0; r < data.size(); r++)
    {
        ...
        while(reader.next() == true)
        {
            ...
            status_e = abs(PART.getInt("status", 0));
            bool elstatus = (2000 <= status_e) && (status_e < 4000);
     
            for(int i = 0; i < nrows; i++) 
            {
                pid.push_back(PART.getInt("pid", i));
                if (PART.getInt("pid", i) == 2212) {ppindex = i; pcount++; status_p = abs(PART.getInt("status", i));}
                if (PART.getInt("pid", i) == 211) {pippindex = i; pipcount++;}
                if (PART.getInt("pid", i) == -211) {pimpindex = i; pimcount++; status_pim = abs(PART.getInt("status", i));}
                if (PART.getInt("pid", i) == 0) {unid++;}   // Uneidentified particles counter
            ...
            if (pid[0] == 11 && // First registered particle should be electron
                elstatus &&     // Status cut (electron in FD)
                fd + cd == nrows && // All particles should be registered in FD or CD
                pcount == 1 &&  // Only one proton
                pipcount == 0 &&    // No pi+
                unid == 0 &&    // No uneidentified particles
                pimcount == 1 &&    // Only one pi minus
                charge == 3) // Three charged particles
            {
                ...
                */
                ...
                if (PART.getFloat("chi2pid", 0) >= 5) continue; // Chi-Squared PID cut
                if (el.Rho() < 1.0 || el.Rho() > 6.535) continue; // Minimum momentum cut
                if((beam + pin - el).M() < 1.2) continue; // W > 1.2 GeV, 2 pion threshold

                double vz_el = PART.getFloat("vz", 0);  // Z-vertex
                if(vz_el < -10 || vz_el > 2) continue;  // Vertex position cut
            
                double hit_el = 0.; // Time of hit
                double time_i, path_i;
                int index_i;

                for (int i = 0; i < Scint.getRows(); i++)
                {
                    index_i = Scint.getInt("pindex", i);
                    time_i = Scint.getFloat("time", i);
                    path_i = Scint.getFloat("path", i);

                    if (index_i == 0 && time_i > hit_el)    {hit_el = time_i; time_e = time_i; path_e = path_i;}

                    if (index_i == ppindex)   {time_p = time_i; path_p = path_i;}
                    if (index_i == pimpindex)   {time_pim = time_i; path_pim = path_i;}
                }
                ...
                if ((EC_in/el.Rho()) < (-0.84 * E_PCAL/el.Rho() + 0.17)) continue;  // Pi minus contamination cut
    ...
    TFile* hfile =  new TFile("MPPT_events_simulation_selection_with_additional_values_saved.root", "RECREATE");
    tree.Write();
